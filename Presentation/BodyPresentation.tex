...
\vfill
\begin{itemize}
  \item \emph{Sense}, \emph{Think}, \emph{Act} paradigm
  \item Potential-fields approach \cite{howard2002mobile}
\end{itemize}
...
\begin{enumerate}
  ...
  \item If the \textbf{nest} has been \textbf{left}, and \textbf{exactly one} chain beacon has been \textbf{sensed} at a distance greater than $d_{c}$, stop and become a \emph{chain end (E)}.
...
\begin{tikzpicture}[scale=0.2,shorten >=1pt,node distance=3cm,on grid,auto] 
   ...
   \node [draw,rectangle,text width=4.5cm] (box) at (0,-26) {%
    ...
    (CE) edge node [draw,rectangle,rounded corners]  {$n_b > 1$} (CM)
