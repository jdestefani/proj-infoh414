...
\begin{Large}
\begin{itemize}
  \item<2-> \emph{Sense}, \emph{Think}, \emph{Act} paradigm
  \vskip15pt
  \item<3-> Potential-fields approach \cite{howard2002mobile}
\end{itemize}
\end{Large}
...
\begin{frame}[t,fragile]{\textbf{Rules}}
\begin{Large}
\begin{enumerate}
  \item Chain beginning rule
  \vskip7pt
  \item Chain building rule
  \vskip7pt
  \item \emph{Chain end} to \emph{Chain member} transition
  \vskip7pt
  \item \emph{Chain member} to \emph{Chain junction} transition
  \vskip7pt
\end{enumerate}
\end{Large}
\note{
...
\begin{frame}[t,fragile]{\textbf{Chain beginning rule}}
\begin{tiny}
%\begin{center}
...
\begin{frame}[t,fragile]{\textbf{Chain building rule}}
\begin{tiny}
%\begin{center}
\begin{tikzpicture}[scale=0.2,shorten >=1pt,node distance=3cm,on grid,auto] 
   \node[state,initial] (EN)   {Exit Nest}; 
...
\begin{frame}[t,fragile]{\textbf{End to Member}}
\begin{tiny}
%\begin{center}
\begin{tikzpicture}[scale=0.2,shorten >=1pt,node distance=3cm,on grid,auto] 
   \node[state,initial] (EN)   {Exit Nest}; 
...
\begin{frame}[t,fragile]{\textbf{Member to Junction}}
\begin{tiny}
%\begin{center}
\begin{tikzpicture}[scale=0.2,shorten >=1pt,node distance=3cm,on grid,auto] 
   \node[state,initial] (EN)   {Exit Nest}; 
   ...
   \node [draw,rectangle,text width=4.5cm] (box) at (24,-26) {%
    ...
    \node [draw,rectangle,text width=4.5cm] (box) at (0,-26) {%
    If a \textbf{chain member (M)} has \textbf{more than two} neighboring \textbf{beacons}, then it changes its state to \emph{chain junction (J)}.
    };
    \path[->] 
    (EN) edge [bend right]  node [left,text width=2cm]  {Out of nest $\wedge \text{ } n_b = 0$} (EX)
         edge [bend left]  node [above]  {$n_b >0$} (EC)
    ...
    (CE) edge node {$n_b > 1$} (CM)
           edge [bend left] node [below left,text width=0.7cm]  {$d_{cb} < 30[cm]$} (EC)
    (CM) edge [bend left] node [red,text width=2cm] {$n_b > 2$} (CJ)
    (EC) edge  node [above]  {In Nest} (EN)
           edge  node [above,rotate=45]  {$n_b > 1 \text{ } \wedge$ Out of Nest} (EX) 
           edge [bend left] node [right,text width=1.3cm] {$n_b = 1 \text{ } \wedge$ Out of Nest $\wedge \text{ } d_{cb} > d_c$} (CE)
           edge  node {On spot} (SR);
...
\begin{frame}[t,fragile]{\textbf{Probabilistic FSM}}
\begin{tiny}
%\begin{center}
\begin{tikzpicture}[scale=0.2,shorten >=1pt,node distance=3cm,on grid,auto] 
   \node[state,initial] (EN)   {Exit Nest}; 
...
\end{minipage}
\vfill
\captionof{figure}{Observed distribution of the number of robots in chain over 50 trials displayed as histogram (a) and empirical cumulative density function (b)}
...
\end{minipage}
\vfill
\captionof{figure}{Observed distribution of the experiments' completion times over 50 trials displayed as histogram (a) and empirical cumulative density function (b)}
\note{
\begin{scriptsize}
\begin{center}
\begin{tabular}{|l|c|c|}
...
\captionof{figure}{Scatterplot of the experiments' completion times versus the number of robots in chain on 50 trials. $r=0.7934599$}
\end{center}
\note{
\begin{center}
\begin{tabular}{|l|c|c|}
...
\begin{itemize}
  ...
  \end{itemize}
  \vskip10pt
  \item Here, simplicity entails:
  \begin{itemize}
    \item Lack of placement optimality
    \item High results variability
  \end{itemize}
  \vskip10pt
  \item The width of the communication range impacts on:
  \begin{itemize}
    \item Completion time
    \item Number of robots in chain
  \end{itemize}
  \vskip10pt
  \item Relevant impact of the structure of the environment on the method's performance.
\end{itemize}
\end{large}
